...
export class TruckRouteMapComponent extends React.Component<MapProps, MapState> {
  ...
  render() {
    ...
    return (
      <Container fixed>
        {this.props.allowChangeLocation &&
          <ChangeUserLocationButton showMap={this.state.openChangeLocationDialog}
                                    setShowMap={this.openCloseLocationDialog} />}
        {!this.state.openChangeLocationDialog && <LoadScript googleMapsApiKey={key as string}>
          <GoogleMap
            mapContainerStyle={{
